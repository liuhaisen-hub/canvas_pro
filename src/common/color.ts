...
function parseAngleArg(arg: string) {
...
function parseNumberPercentageArg(
  arg: string,
  opts: { min: number; max: number; percentageRequired?: boolean }
) {
...
function parseComponentFunctionArgs(args: string) {
...
export default function paserCssToGlColor(cssStr: string): number[] {
  ...
  if (cssStr.charAt(0) === '#') {
    ...
    if (matches == null) {
      ...
      if (cssStr in x11ColorTable) {
        return x11ColorTable[cssStr as keyof typeof x11ColorTable]
      } else {
        throw new Error('Bad Color')
      }
    } else {
...
export const cssToGlColor = (cssStr: string) => {
  try {
    return paserCssToGlColor(cssStr)
  } catch (e) {
